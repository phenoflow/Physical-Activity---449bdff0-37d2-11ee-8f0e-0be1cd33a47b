...
(async () => {
  ...
  await fs.writeFile("physical-activity-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("physical-activity-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
